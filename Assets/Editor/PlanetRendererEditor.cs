...
public class PlanetRendererEditor : Editor {
  ...
  int previewSize = (int)PreviewSize.Medium;
  ...
  private void updatePreview() {
    PlanetRenderer planetRenderer = target as PlanetRenderer;
    WorldGenerator generator = new WorldGenerator(planetRenderer.worldSettings);

    if (planetRenderer.worldSettings != null) {
      // Generate the height values
      float[,] heightValues = Noise.generateNoiseMap(previewSizePixels, previewSizePixels, planetRenderer.worldSettings.geography, new Vector2(worldPos.x, worldPos.y));
      float[,] tempValues = Noise.generateNoiseMap(previewSizePixels, previewSizePixels, planetRenderer.worldSettings.temperature, new Vector2(worldPos.x, worldPos.y));
      float[,] rainfall = Noise.generateNoiseMap(previewSizePixels, previewSizePixels, planetRenderer.worldSettings.rainfall, new Vector2(worldPos.x, worldPos.y));
      ...
      for (int y = 0; y < previewSizePixels; y++)
        for (int x = 0; x < previewSizePixels; x++) {
          ...
          float temperature = generator.getTemp(tempValues[x, y], height);
          float rianfall = rainfall[x, y];

          BiomeType biomeType = generator.getBiomeType(temperature, rianfall, height);
          Color biomeColor;
          ...
          if (height <= planetRenderer.worldSettings.seaLevel) {
  ...
  public override void OnInspectorGUI() {
    ...
    if (geographyMap != null && showPreview) {
      ...
      BiomeData biom = generator.queryBiom(worldPos.x, worldPos.y, previewSizePixels);
